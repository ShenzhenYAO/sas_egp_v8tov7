/* To create a project xml from the prototype 
    ...
    adding eg task
    1. in ProjectCollection.Elements.Element(of the parent PFD).PFD
        make and append a Process component -- Same as making CodeTask
    2. in ProjectCollection.Elements, add an Element Type="SAS.EG.ProjectElements.EGTask"
        (with a special child node: EGTask) -- Similar to but different from making CodeTask
    3. in ProjectCollection.External_Objects.ProjectTreeView.EGTreeNode (of the Parent PFD)
        add an EGTreeNode (Note. Unlike CodeTasks, EGTreeNode of EGTask is not add under the EGTreeNode of Programs)
        -- Similar to adding a link or a shortcuttofile
    4. in ProjectCollection.External_Objects.ProcessFlowView.Graphics
        add a TaskGraphic
        -- Same as making CodeTask
    5. add the xml code to the egp zip (using the EGTask Element.ID as the folder name and the xml file name)
...
(async () => {
    ...
    // let pdf_arr = [{ 'Label': 'PFD1' }], pfd_input = [], config_pfd = []
...
like data f; set g; run;`},
    ...
    for (let i = 0; i < egtask_input_arr.length; i++) {
        ...
        doms_obj = await make_append_egtask_component(doms_obj, config_egtask[i], newZip)
    } // for(let i=0;i<task_input_arr.length;i++)
...
async function make_append_egtask_component(doms_obj, config_egtask, newZip) {
    ...
    // 5. add the program text and insert into the egp zip (only do it when the config_egtask.code is not null, e.g., the egtask is not a shortcut to an exteranl sas program)
    if (config_egtask.xmlstr && config_egtask.xmlstr !== '') {
        ...
        newZip.addFile(config_egtask.Element.ID + '\\' + config_egtask.Element.ID + '.xml', Buffer.from(egtask_xmlstr, "utf-8"))
    } // config_egtask.code
